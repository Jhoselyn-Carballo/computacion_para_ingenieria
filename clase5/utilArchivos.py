# -*- coding: utf-8 -*-
"""
Created on Fri Feb  4 06:55:42 2022

@author: JHOSS
"""

# Manejar archivos
"""
listaNumeros.txt
1 
2
3<-
4
5
"""
# leer Linea por linea un archivo
numeros = abierto ('listaNumeros.txt', 'r')
print(numeros. readline()) # imprime 1 en la pantalla
numeros. línea de lectura()
print(numeros. readline())# imprime 3

# Leer lineas de un archivo  y da como resulta un array ['','']

res = números. readlines() # retorna un array con el contenido del archivo
print(res))

#anexa archivos

archivo = abierto ('listaProfes.txt', 'a')
archivo. write('nuevo DOcente') # agrea nuevas lineas a un archivo
archivo. cerrar()